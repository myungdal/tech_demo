// ============================================================================
// myungdal tech-demo project
// 
// Author: 안명달 (Myungdal Ahn)
// Email: mooondal@gmail.com
// GitHub: https://github.com/myungdal/tech_demo.git
// ============================================================================

#pragma once

#include "ServerEngine/Socket/SocketImpl/HostSocket.h"
#include "ServerEngine/Socket/RequestSocket.h"


class SocketDbFromFront;

class SocketDbFromGame final : public RequestSocket<SocketDbFromGame, MAX_POOL_SIZE_FACTOR_SOCKET_DB_FROM_GAME, HostSocket>
{
protected:
	void OnHandshakeCompleted(MAYBE_UNUSED bool reconnected);
	void OnLostSocket();

protected:
	bool OnDispatchPacket(NetworkPacket& rp) final;
	void DispatchPacket(NetworkPacket& rp);
	void DispatchPacket_async(MAYBE_UNUSED SocketPtrType& ptr, PacketTemp tp);
	void DispatchPacketToHandler(NetworkPacket& rp);
};
