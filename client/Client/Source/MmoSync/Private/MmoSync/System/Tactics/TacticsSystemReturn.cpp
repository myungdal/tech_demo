...
#include "MmoSync/System/Tactics/TacticsSystemReturn.h"


TacticsSystemReturn::TacticsSystemReturn(Engine& engine, TimeStepUpdateCycle timeStepUpdateCycle, GameInstance& game)
	:
	TacticsSystemBase<TacticsSystemReturn>(engine, timeStepUpdateCycle, game)
{
	...
	IncludeComponent<TacticsComponentReturn>();
	ExcludeComponent<MotionComponentGroupA>();
	ExcludeComponent<MotionComponentGroupB>();
	ExcludeComponent<MotionComponentGroupC>();
	ExcludeComponent<PcComponent>();  // PC 전체 제외 (Leader + Follower) - PcSystem::MoveFollowers에서 이동 제어
}
void TacticsSystemReturn::Update()
{
	if (Data().empty())
		return;
	...
	for (EntityIdp entityIdp : Data())
	{
		...
		TacticsComponentReturn& state0 = GetComponent<TacticsComponentReturn>(entityIdp.first);
