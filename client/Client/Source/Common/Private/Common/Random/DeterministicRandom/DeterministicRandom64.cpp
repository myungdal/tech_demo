...
DeterministicRandom64::DeterministicRandom64(uint64_t seed)
	:
	mSeed(seed)
{
}
uint64_t DeterministicRandom64::operator()()
{
...
uint64_t DeterministicRandom64::operator()(uint64_t range)
{
	...
	const uint64_t threshold = static_cast<uint64_t>(-static_cast<int64_t>(range));
...
int64_t DeterministicRandom64::operator()(int64_t min, int64_t max)
{
	// [EDGE-CASE] min > max인 경우 교환
	if (min > max)
		std::swap(min, max);

	uint64_t diff = static_cast<uint64_t>(max) - static_cast<uint64_t>(min) + 1ull;
